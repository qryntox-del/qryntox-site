...
function ProductsPage() {
    ...
    React.useEffect(() => {
        const handleLiveSearch = (e) => setSearchQuery(e.detail);
        window.addEventListener('liveSearch', handleLiveSearch);
        return () => window.removeEventListener('liveSearch', handleLiveSearch);
    }, []);

    const loadProducts = async () => {
        let hasCache = false;
        try {
            const cachedProducts = localStorage.getItem('qryntox_local_products');
            if (cachedProducts) {
                try {
            ...
            try {
                ...
                console.warn('Background fetch warning safely handled:', fetchErr.message || fetchErr);
                res = { items: [] };
            }

            let finalProducts = res?.items || [];
            if (finalProducts.length > 0) {
                ...
                setError(null);
                localStorage.setItem('qryntox_local_products', JSON.stringify(finalProducts));
            } else if (!hasCache) {
                setError("Network issue detected. Showing available cache.");
            }
        } catch(e) {
            console.warn('Unexpected error in loadProducts gracefully caught:', e.message);
        } finally {
            setLoading(false);
        }
    };
    ...
    try {
        ...
        activeProducts = safeProducts.filter(p => p?.objectData?.status !== 'Draft');
        categories = ['All', ...new Set(activeProducts.map(p => p?.objectData?.category).filter(Boolean))];
        
        filteredProducts = activeProducts.filter(p => {
            const category = p?.objectData?.category || '';
            const name = p?.objectData?.name || '';
            const description = p?.objectData?.description || '';
        ...
        console.warn('Product filtering fallback:', error);
        filteredProducts = [];
    }

    return (
        <div className="container mx-auto px-5 md:px-6 py-8 md:py-12" data-name="products-page" data-file="components/ProductsPage.js">
            <div className="flex flex-col md:flex-row justify-between items-center md:items-end mb-12 gap-6 animate-slide-up text-center md:text-left mx-auto">
                <div className="mx-auto md:mx-0 flex flex-col items-center md:items-start">
                    <h1 className="text-4xl md:text-5xl font-sans font-bold mb-4 text-black uppercase tracking-widest">THE COLLECTION</h1>
                    <p className="text-gray-600 max-w-xl text-sm leading-relaxed">
                        Meticulously crafted silhouettes. Built for the permanent collective.
                    </p>
                </div>
            ...
            {loading || filteredProducts.length === 0 ? (
                <div className="py-32 flex justify-center items-center flex-col gap-4">
                    <i className="icon-loader animate-spin text-4xl text-gray-300"></i>
                </div>
            ) : error ? (
                <div className="py-24 text-center text-red-500 font-medium bg-gray-50 rounded-xl border border-gray-200"><i className="icon-circle-alert text-3xl mb-3"></i><p>{String(error)}</p></div>
            ) : (
                <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-[15px] md:gap-8 animate-slide-up stagger-1">
                        ...
                        return (
                            <div key={product.objectId || product.id || data.name} className="group cursor-pointer flex flex-col h-full bg-white" onClick={() => window.location.href = `product.html?id=${product.objectId || product.id}`}>
                                <div className="relative aspect-[3/4] overflow-hidden bg-[#F2F2F2] mb-4 flex items-center justify-center">
                                    ...
                                    {/* Bottom Shadow Overlay on Hover */}
                                ...
                                <div className="flex justify-between items-start px-1 pb-2 mt-2 flex-1">
                                    <div className="flex flex-col text-left pr-2 md:pr-4 overflow-hidden">
                                        <h3 className="font-serif text-sm font-bold text-black mb-1 capitalize tracking-wide truncate">{String(data.name || '')}</h3>
                                        {data.sizes && Array.isArray(data.sizes) && data.sizes.length > 0 && (
                                            <p className="font-sans text-[10px] font-medium text-gray-500 uppercase tracking-widest truncate">
                                                ALL - {data.sizes.join(', ')}
                                            </p>
                                        )}
                                    </div>
                                    <div className="text-right shrink-0 ml-2">
                                        <p className="text-xs font-bold text-black whitespace-nowrap">{String(data.price || '').replace('$', '₹')}</p>
                                    </div>
                                </div>
                            </div>
                        );
